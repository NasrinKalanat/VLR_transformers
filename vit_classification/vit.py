...
class ViT(nn.Module):
    ...
    def __init__(self, patch_dim, d_model, d_ff, num_heads, num_layers, num_patches, num_classes, device = 'cuda'):
        ...
        self.cls_token = nn.Parameter(torch.randn(1, d_model, device=device), requires_grad=True) # TODO (learnable [CLS] token embedding)
    ...
    def patchify(self, images):
        ...
        images=nn.Unfold(kernel_size=(self.patch_dim , self.patch_dim), stride=self.patch_dim)(images)
    ...
    def forward(self, images):
        ...
        patches_embedded = torch.cat((self.cls_token.repeat(images.shape[0],1,1),patches_embedded),1) # TODO (append a CLS token to the beginning of the sequence of patch embeddings)
