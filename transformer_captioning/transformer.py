...
class AttentionLayer(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        dot_product = torch.matmul(query,key.transpose(1,2))/np.sqrt(self.embed_dim)
        ...
        y = self.dropout(torch.softmax(dot_product,dim=-1))
...
class MultiHeadAttentionLayer(AttentionLayer):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        query = query.reshape(N,S,H,int(D//H)).permute(0,2,1,3)
        key = key.reshape(N,T,H,int(D//H)).permute(0,2,1,3)
        value = value.reshape(N,T,H,int(D//H)).permute(0,2,1,3)
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        ...
        pos=torch.arange(0,S,device=x.device,dtype=torch.long) #? .repeat(N,1,1)
